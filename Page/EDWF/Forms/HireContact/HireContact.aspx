<%@ Page Title="" Language="C#" MasterPageFile="~/Page/EDWF/Forms/Control.Master" AutoEventWireup="true" CodeBehind="HireContact.aspx.cs" Inherits="ERP.LHDesign2020.Page.EDWF.Forms.HireContact.HireContact" %>

<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <script src="../../../../js/Numeric.js"></script>
    <script>
        var This = "HireContact.aspx";
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if ($('#HdINQ').val() == "1") { //Inquiry
                        ...
                        $('#Spdocumentno').html(response.d['Documentno']);
                        $('#TxtVendorname').attr('data-value', response.d['VendorId']);
                        $('#TxtVendorname').val(response.d['fullname']);
                        $('#TxtVendoraddress').val(response.d['address']);
                        $('#TxtVendortel').val(response.d['Tel']);
                        $('#Txtcontactdate').val(response.d['Contactdate']);
                        $('#TxtVendorcardID').val(response.d['Cardid']);
                        $('#TxtVendorexprirydate').val(response.d['Expirydate']);
                        $("#CbVendorbank").val(response.d['Bankid']);
                        $('#TxtVendorbankaccountno').val(response.d['Bankaccountno']);
                        $('#TxtVendorbankaccountname').val(response.d['Bankaccountname']);
                        $("#CbVendorbankaccounttype").val(response.d['Bankaccounttype']);
        ...
        function Getbank() {
            $.ajax({
                ...
                success: function (response) {
                    res = response.d;
                    $('#CbnewVendorbank').find('option').remove().end();
                    for (i = 0; i < response.d.length; i++) {
                        val = response.d[i];
                        $('#CbnewVendorbank').append($('<option/>', {
                            value: val["Val"],
                            text: val["Name"]
                        }));
                    }
                    $('#CbVendorbank').find('option').remove().end();
                    for (i = 0; i < response.d.length; i++) {
                        val = response.d[i];
                        $('#CbVendorbank').append($('<option/>', {
                            value: val["Val"],
                            text: val["Name"]
                        }));
                    }
        ...
        function Custom(ctrl, Panel) {
            ...
            if (ctrl == 'GvVendor') {
                Getbank();
                $('#DivVendor').modal('hide');
                $('#DivnewVendor').modal('show');
            }
        }
        function doSaveVendor(json) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('#DivVendor').modal('show');
                    $('#DivnewVendor').modal('hide');
                    SearchVendor();
                },
                async: true,
                error: function (er) {
        ...
        function SaveVendor() {
            var json = '';
            if ($('#TxtnewVendorname').val() == '') {
                Msgbox('โปรดระบุชื่อ Outsource');
                return;
            }
            if ($('#TxtnewVendortel').val() == '') {
                Msgbox('โปรดระบุเบอร์โทรศัพท์ Outsource');
                return;
            }
            if ($('#TxtnewVendoraddress').val() == '') {
                Msgbox('โปรดระบุที่อยู่ Outsource');
                return;
            }
            if ($('#TxtnewVendorcardID').val() == '') {
                Msgbox('โปรดระบุบัตรประชาชน Outsource');
                return;
            }
            ...
            json += 'CbnewVendorbankaccounttype :' + $('#CbnewVendorbankaccounttype').val() + '|';

            $.ajax({
                type: "POST",
                url: This + "/ValidateVendor",
                data: "{'json' :'" + json + "'}",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (response) {
        ...
        function SearchVendor() {
            ...
            ClearResource('GvVendor');
            var Columns = ["ชื่อ Outsource!C", "ที่อยู่!L"];
            var Data = ["fullname", "Address"];
            var Searchcolumns = ["ชื่อ Outsource", "ที่อยู่"];
            var SearchesDat = ["fullname", "Address"];
            var Width = ["40%", "60%"];
            var grdVendor = new Grid(Columns, SearchesDat, Searchcolumns, 'GvVendor', 30, Width, Data, "", '', '2', 'สร้าง Outsourceใหม่', '', '', '', '', '', 'id', 'id', '', 'id', '', '');
            $('#DivVendorcont').html(grdVendor._Tables());
            grdVendor._Bind();
            ...
            if (ctrl == 'GvVendor') {
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('#TxtVendorname').attr('data-value', response.d[0]['Id']);
                        $('#TxtVendorname').val(response.d[0]['Fullname']);
                        $('#TxtVendoraddress').val(response.d[0]['Address']);
                        $('#TxtVendortel').val(response.d[0]['Tel']);
                        $('#TxtVendorcardID').val(response.d[0]['CardID']);
                        $('#TxtVendorexprirydate').val(response.d[0]['Exprirydate']);
                        $('#CbVendorbank').val(response.d[0]['Bank']);
                        $('#TxtVendorbankaccountname').val(response.d[0]['Bankaccountname']);
                        $('#TxtVendorbankaccountno').val(response.d[0]['Bankaccountno']);

                        $('#CbVendorbankaccounttype').val(response.d[0]['Bankaccounttype']);
                        $('#DivVendor').modal('hide');
                    },
                    async: true,
                    error: function (er) {
        ...
        $(function () {
            ...
            $("#TxtVendorexprirydate").datepicker({
                format: 'dd-mm-yyyy',
                autoclose: true,
                todayHighlight: true
            }).datepicker('update', new Date());
            ...
            $("#TxtnewVendorexprirydate").datepicker({
                format: 'dd-mm-yyyy',
                autoclose: true,
                todayHighlight: true
            }).datepicker('update', new Date());
        ...
        function Save() {
            ...
            json += 'Vendorid :' + $('#TxtVendorname').attr('data-value') + '|';
            json += 'Vendorname :' + $('#TxtVendorname').val() + '|';
            json += 'Vendoraddress :' + $('#TxtVendoraddress').val() + '|';
            json += 'Vendortel :' + $('#TxtVendortel').val() + '|';
            json += 'contactdate :' + $('#Txtcontactdate').val() + '|';
            json += 'VendorcardID :' + $('#TxtVendorcardID').val() + '|';
            json += 'Vendorexprirydate :' + $('#TxtVendorexprirydate').val() + '|';
            json += 'Vendorbank :' + $("#CbVendorbank").val() + '|';
            json += 'Vendorbanktext :' + $("#CbVendorbank option:selected").text() + '|';
            json += 'Vendorbankaccountno :' + $('#TxtVendorbankaccountno').val() + '|';
            json += 'Vendorbankaccountname :' + $('#TxtVendorbankaccountname').val() + '|';
            json += 'Vendorbankaccountype :' + $("#CbVendorbankaccounttype").val() + '|';
            json += 'Vendorbankaccountypetext :' + $("#CbVendorbankaccounttype option:selected").text() + '|';
    ...
    <div class="container-fluid">
        ...
        <div class="row" style="margin-top: 20px;">
            <div class="col-2" style="text-align: right;">
                <span> Outsource / Outsource</span>&nbsp<span class="sp-require">*</span>
            </div>
            <div class="col-5">
                <div class="input-group mb-3">
                    <input type="text" id="TxtVendorname" data-value="" class="form-control" placeholder="กดค้นหา Outsource" style="font-size: 14px; border-radius: 1px;" readonly="readonly">
        ...
        <div class="row">
            ...
            <div class="col-5">
                <div class="input-group mb-3">
                    <textarea class="form-control" id="TxtVendoraddress" data-value="" readonly="readonly" style="height: 50px;"></textarea>
                </div>
            </div>
            <div class="col-2" style="text-align: right;">
            ...
            <div class="col-3" style="text-align: right;">
                <input type="text" id="TxtVendortel" onkeypress="return isNumberKey(event)" class="form-control" />
            </div>
        </div>
        <div class="row">
            <div class="col-2" style="text-align: right;">
            ...
            <div class="col-5">
                <div class="input-group mb-3">
                    <input type="text" id="TxtVendorcardID" onkeypress="return isNumberKey(event)" class="form-control" />
            ...
            <div class="col-3" style="text-align: right;">
                <div class='input-group date' id='DtpVendorexprirydate'>
                    <input id="TxtVendorexprirydate" readonly="readonly" class="form-control"  style="cursor:pointer;"  type="text" />
                    <span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>
                </div>
        ...
        <div class="row">
            ...
            <div class="col-5">
                <select id="CbVendorbank" class="form-control" style="margin-bottom: 10px;">
                </select>
            </div>
            <div class="col-2" style="text-align: right;">
            ...
            <div class="col-3" style="text-align: right;">
                <div class="input-group mb-3">
                    <input type="text" id="TxtVendorbankaccountno" onkeypress="return isNumberKey(event)" class="form-control" />
                </div>
            </div>
        ...
        <div class="row" style="margin-bottom: 20px;">
            ...
            <div class="col-5">
                <input type="text" id="TxtVendorbankaccountname" class="form-control" />
            </div>
            <div class="col-2" style="text-align: right;">
                <span>ประเภท</span>&nbsp<span class="sp-require">*</span>
            </div>
            <div class="col-3" style="text-align: right;">
                <select id="CbVendorbankaccounttype" class="form-control">
    ...
    <div class="modal" id="DivVendor" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog modal-lg" id="Divprofilecontent">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="container" id="DivVendorcont" style="border: solid 1px lightgray; margin-right: 10px; min-height: 320px; padding: 8px;">
                    </div>
                </div>
                <!-- Modal footer -->
    ...
    <div class="modal" id="DivnewVendor" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <span> Outsourceใหม่</span>
                </div>
                <!-- Modal body -->
                <div class="modal-body">
                    <div class="container" id="DivnewVendorcont" style="margin-right: 10px; min-height: 320px; padding: 8px;">
                        <div class="row">
                            <div class="col-6">
                                <span>ชื่อ Outsource</span>&nbsp;<span style="color: red;">*</span>
                            </div>
                            <div class="col-6">
                        ...
                        <div class="row">
                            <div class="col-6">
                                <input type="text" class="form-control" id="TxtnewVendorname" style="border-radius: 1px;" />
                            </div>
                       
                            <div class="col-6">
                                <input type="text" onkeypress="return isNumberKey(event)" class="form-control" id="TxtnewVendortel" style="border-radius: 1px;" />
                            </div>
                        ...
                        <div class="row">
                         ...
                         <div class="col-6">
                                <input type="text" maxlength="13" onkeypress="return isNumberKey(event)" class="form-control" id="TxtnewVendorcardID" style="border-radius: 1px;" />
                            </div>
                      
                            <div class="col-6">
                                <div class='input-group date' id='DtpnewVendorexprirydate'>
                                    <input id="TxtnewVendorexprirydate" class="form-control" type="text" />
                                    <span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>
                                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-6">
                                <select class="form-control" id="CbnewVendorbank">
                                </select>
                            </div>
                            <div class="col-6">
                                <select class="form-control" id="CbnewVendorbankaccounttype">
                                    <option value="SV" selected>ออมทรัพย์</option>
                                    <option value="CA">กระแสรายวัน</option>
                                </select>
                            </div>
                        ...
                        <div class="row">
                              <div class="col-6">
                                <input type="text" class="form-control" onkeypress="return isNumberKey(event)" id="TxtnewVendorbankaccountno" style="border-radius: 1px;" />
                            </div>
                            <div class="col-6">
                                <input type="text" class="form-control" id="TxtnewVendorbankaccountname" style="border-radius: 1px;" />
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <textarea class="form-control" id="TxtnewVendoraddress" style="border-radius: 1px; height: 100px;"></textarea>
                            </div>
                        </div>
                    ...
                    <!-- Modal footer -->
                    <div class="modal-footer">
                        <button type="button" class="btn btn-standard" style="background-color:#313131;color:white; font-size: 14px; border-radius: 0;"  onclick="SaveVendor();">บันทึก Outsourceใหม่</button>
